...
import {
  BookMarked, Search, X, Sparkles, Check, AlertCircle, Loader, Circle,
  ChevronLeft, ChevronRight, RotateCcw, BookOpen, Calendar, TrendingUp, Award, Flame,
} from "lucide-react";
import { ViewHeader } from "@/components/shared/view-header";
import { SectionCard } from "@/components/shared/section-card";
import { ProgressRing } from "@/components/shared/progress-ring";
import { AnimatedNumber } from "@/components/shared/animated-number";
import { IslamicPatternMoroccan, IslamicPatternArabesque } from "@/components/shared/islamic-pattern";
...
import { useHifz, useUpdateHifz, type HifzStatus, type HifzSurahItem } from "@/lib/hooks";
...
function StatusIcon({ status, className }: { status: HifzStatus; className?: string }) {
  switch (status) {
    case "memorized": return <Check className={className} />;
    case "in_progress": return <Loader className={className} />;
    case "needs_review": return <AlertCircle className={className} />;
...
export function HifzView() {
  ...
  const [statusFilter, setStatusFilter] = React.useState<HifzStatus | "all">("all");
  const [revFilter, setRevFilter] = React.useState<(typeof REVELATION_FILTERS)[number]["id"]>("all");
    ...
    update.mutate(
      { surahNumber: surah.number, status },
      {
        onSuccess: () => {
          ...
          setSelected((prev) => (prev && prev.number === surah.number ? { ...prev, status } : prev));
        },
      }
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <SectionCard className="lg:col-span-2 relative overflow-hidden">
          ...
          <div className="relative flex flex-col sm:flex-row items-center gap-6">
            ...
            <div className="flex-1 space-y-3 w-full">
              <div>
                <h3 className="text-display text-lg font-medium">Perjalanan Menghafal</h3>
                <p className="text-sm text-muted-foreground">
                  {stats?.totalMemorizedAyahs.toLocaleString() ?? 0} dari {stats?.totalQuranAyahs.toLocaleString() ?? 6236} ayahs Terkhafal
                </p>
              </div>
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="rounded-xl bg-amber-500/10 px-3 py-2 text-center">
                  ...
                  <p className="text-[10px] text-muted-foreground">Sedang Proses</p>
                </div>
                <div className="rounded-xl bg-rose-500/10 px-3 py-2 text-center">
                  ...
                  <p className="text-[10px] text-muted-foreground">Perlu Diulang</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <SectionCard className="relative overflow-hidden">
          <IslamicPatternArabesque className="text-amber-500" opacity={0.06} />
          <div className="relative">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-display text-base font-medium">Murajaah</h3>
              </div>
            </div>
            <div className="space-y-3">
              <div className="flex items-baseline justify-between">
                <span className="text-sm text-muted-foreground">Perlu diulang</span>
              ...
              <div className="flex items-baseline justify-between">
                <span className="text-sm text-muted-foreground">Sudah diulang minggu ini</span>
                ...
                return (
                  <Tooltip key={surah.number}>
                    <TooltipTrigger asChild>
                      <motion.button
                        ...
                        className={cn(
                          ...
                          surah.status === "memorized" && "bg-emerald-500/80 hover:bg-emerald-500",
                          surah.status === "in_progress" && "bg-amber-500/80 hover:bg-amber-500",
                          surah.status === "needs_review" && "bg-rose-500/80 hover:bg-rose-500",
                          surah.status === "not_started" && "bg-muted-foreground/20 hover:bg-muted-foreground/30"
                        )}
                    ...
                    <TooltipContent side="top" className="text-xs">
                      <div className="text-center">
                        <p className="font-medium">{surah.number}. {surah.name}</p>
                        <p className="text-arabic text-base leading-tight">{surah.arabic}</p>
                        <p className="text-muted-foreground text-[10px]">
                          {surah.ayahs} ayahs · {surah.revelation === "meccan" ? "Makkiyah" : "Madaniyah"} · {meta.label}
                        </p>
                      </div>
                    </TooltipContent>
                  </Tooltip>
            ...
            return (
              <motion.button
                ...
                className={cn(
                  ...
                  surah.status === "memorized" && "border-emerald-500/30 bg-emerald-500/5",
                  surah.status === "in_progress" && "border-amber-500/30 bg-amber-500/5",
                  surah.status === "needs_review" && "border-rose-500/30 bg-rose-500/5",
                  surah.status === "not_started" && "border-border/70 bg-card hover:border-border"
                )}
              >
                ...
                <p className="text-arabic text-lg leading-none mb-1.5 text-foreground">{surah.arabic}</p>
                <p className="text-xs font-medium truncate">{surah.name}</p>
                <div className="flex items-center justify-between mt-2">
                  <span className="text-[10px] text-muted-foreground tabular-nums">{surah.ayahs} ayahs</span>
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div>
              <p className="text-[11px] uppercase tracking-wider text-muted-foreground mb-1">
                Surah {surah.number} · {surah.revelation === "meccan" ? "Makkiyah" : "Madaniyah"} · {surah.ayahs} ayahs
              </p>
              <h2 className="text-display text-2xl font-medium">{surah.name}</h2>
              <p className="text-sm text-muted-foreground">{surah.english}</p>
              <p className="text-arabic text-3xl mt-2 text-foreground">{surah.arabic}</p>
            </div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      active
                        ? cn(m.bg, m.ring, "ring-1", m.color, "border-transparent")
